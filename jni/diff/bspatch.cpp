...
void errx(char a, const char* format, char* param)
{
    printf(format,param);
    exit(1);
}
void errx(char a, const char* format)
{
    printf("%s",format);
    exit(1);
}
...
JNIEXPORT void JNICALL Java_com_ziyu_ndkupdate_MainActivity_patchApk
  (JNIEnv *env, jclass jcls, jstring jold_path, jstring jinstall_path, jstring jpatch_path){

	char* old_path = (char*)env->GetStringUTFChars(jold_path, NULL);
	char* new_path = (char*)env->GetStringUTFChars(jinstall_path, NULL);
	...
	env->ReleaseStringUTFChars(jinstall_path, new_path);
