...
#include "com_ziyu_ndkupdate_MainActivity.h"
...
void err(char a, const char* format, char* param)
{
    printf(format,param);
    exit(1);
}
void err(char a, const char* format)
{
    printf("%s",format);
    exit(1);
}
...
//传入文件路径，调用diff main方法
JNIEXPORT void JNICALL Java_com_ziyu_ndkupdate_MainActivity_diffApk
  (JNIEnv *env, jclass jcls, jstring jold_path, jstring jnew_path, jstring jpatch_path){

	char* old_path = (char*)env->GetStringUTFChars(jold_path, NULL);
	char* new_path = (char*)env->GetStringUTFChars(jnew_path, NULL);
	char* patch_path = (char*)env->GetStringUTFChars(jpatch_path, NULL);

	LOGI("old -> %s",jold_path);
	LOGI("new -> %s",jnew_path);
	LOGI("patch -> %s",jpatch_path);
	...
	env->ReleaseStringUTFChars(jold_path, old_path);
	env->ReleaseStringUTFChars(jnew_path, new_path);
	env->ReleaseStringUTFChars(jpatch_path, patch_path);
